  ...
  function exportPDF() {
    ...
    try {
      ...
      const body = transactions.map((r) => [
        ...
        r.ReferenceNumber || "",
        r.Description,
        formatAmount(r.Debit_Amount) || "0.00",
        formatAmount(r.Credit_Amount) || "0.00",
        formatAmount(r.RunningBalance) || "0.00",
      ]);
        ...
        <table style={S.table}>
          ...
          <tbody>
            {transactions.map((row) => (
              <tr key={row.StatementOfAccountID}>
                ...
                <td style={S.tdRight}>{formatAmount(row.RunningBalance)}</td>
              </tr>
            ))}
          </tbody>
          ...
          <tfoot>
            <tr>
              ...
              <td style={S.tfootTdRight}>{formatAmount(lastBalance)}</td>
            </tr>
          </tfoot>
        </table>
